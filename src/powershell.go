...
func createStartMenu(games *[]Game) {
	...
	err := os.MkdirAll(*startMenuDir+"VisualElements", os.ModePerm)
	if err != nil {
	...
	for _, game := range *games {
		...
		partialStartLayout += fmt.Sprintf(startLayoutTile,
			*tileSize,
			...
			game.Title)
		tileCount++
		writeFile(*startMenuDir+game.FileName+".bat", `"C:\Program Files (x86)\GOG Galaxy\GalaxyClient.exe" /command=runGame /gameId=`+game.ReleaseKey)
		writeFile(*startMenuDir+game.FileName+".VisualElementsManifest.xml", fmt.Sprintf(visualElements, game.FileName, nameOnLogo))
		if !fileExists(*startMenuDir+"VisualElements\\MediumIcon"+game.FileName+".png") || *force {
			allPowershellDownloadImage += fmt.Sprintf(powershellDownloadImage+"\n", game.IconFileName, *startMenuDir+"VisualElements\\MediumIcon"+game.FileName)
		}
		allPowershellCreateShortcut += fmt.Sprintf(powershellCreateShortcut+"\n", *startMenuDir+game.Title, *startMenuDir+game.FileName)
	}
